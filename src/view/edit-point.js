...
const createTypeItemTemplate = (type) => {
  ...
  items.map((item) => {
    ...
    res.push(`<div class="event__type-item">
      <input id="event-type-${itemLowerCase}-1" class="event__type-input  visually-hidden" type="radio" name="event-type" value="${itemLowerCase}" ${active}>
      <label class="event__type-label  event__type-label--${itemLowerCase}" for="event-type-${itemLowerCase}-1">${item}</label>
    </div>`);
  });
  return res.join('');
};
...
const createEditPointTemplate =  (point) => {
  ...
  const typeItemsTemplate = createTypeItemTemplate(type);
